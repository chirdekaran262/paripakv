...
export default function OrderDetailsBuyer() {
    ...
    const fetchOrderDetails = async (showRefreshing = false) => {
        try {
            ...
            const orderRes = await axios.get(`http://localhost:8089/orders/getById?id=${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            const orderData = orderRes.data;
            setOrder(orderData);

            const listingRes = await axios.get(`http://localhost:8089/listings/byId?id=${orderData.listingId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            const listingData = listingRes.data;
            setListing(listingData);

            const farmerRes = await axios.get(`http://localhost:8089/users/productDetails?id=${listingData.farmerId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            setFarmer(farmerRes.data);

            if (orderData.transporterId) {
                const transporterRes = await axios.get(`http://localhost:8089/users/productDetails?id=${orderData.transporterId}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
